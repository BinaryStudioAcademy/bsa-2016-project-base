...
class ProjectViewService {
	...
	getProject(state, dispatch, dispObj) {
	    fetch(state.CONST_projectsRestPath + '/' + dispObj.selectedProjectId + '/users-owners/',
	...
	getProjectStage(path, dispatch, dispObj) {
    	fetch(rpath + '/' + dispObj.projStageId,constants.cookieMarker)
	    .then(this.checkStatus)
    	.then(this.parseJSON)
    	.then(data => {
	    ...
	    .then(data => {
	    	let sectionsPromises = [];
	      	data.features.forEach(function(entry, i, arr) {
				sectionsPromises.push(fetch(
					dispObj.sectionsPath + '/' + entry.section, constants.cookieMarker
				));
	    	});
